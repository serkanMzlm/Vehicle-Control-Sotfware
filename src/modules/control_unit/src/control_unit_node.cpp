...
void ControlUnit::controlSelection()
{
    if (control_unit == "joy")
    {
        sub.joy = this->create_subscription<joyMsg>("joy", 10, std::bind(&ControlUnit::joyCallback, this, std::placeholders::_1));
    }
    else if (control_unit == "keyboard")
    {
        sub.keyboard = this->create_subscription<int32Msg>("/keypress", 10, std::bind(&ControlUnit::keyboardCallback, this, std::placeholders::_1));
